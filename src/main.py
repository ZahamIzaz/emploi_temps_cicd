    ...
    try:
        ...
        if courses:
            ...
            for i, course in enumerate(courses[:5]):
                ...
                print(f"     Salle: {course.get('salle', 'N/A')} | Prof: {course.get('prof', 'N/A')}")
...
Exemples d'utilisation:
  ...
  python -m wigor_viewer.src.main --log-file wigor.log
    ...
    parser.add_argument(
        '--url',
        type=str,
        help='URL de la page Wigor (requis en mode test)'
    )
    
    parser.add_argument(
        '--cookie',
        type=str,
        help='Cookie d\'authentification (requis en mode test)'
    )
    ...
    parser.add_argument(
        '--log-file',
        type=str,
        help='Fichier de sortie pour les logs (optionnel)'
    )
...
def main():
    ...
    try:
        if args.test:
            ...
            if os.environ.get('CI') or os.environ.get('DISPLAY') == '' or os.environ.get('HEADLESS'):
