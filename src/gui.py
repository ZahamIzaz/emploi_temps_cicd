...
class WigorViewerGUI:
    ...
    def __init__(self):
        ...
        # Variables
        self.url_var = tk.StringVar(value="https://ws-edt-cd.wigorservices.net/WebPsDyn.aspx?Action=posEDTLMS")
    ...
    def _create_widgets(self):
        ...
        # Titre
        title_label = ttk.Label(main_frame, text="Wigor Viewer", 
                               font=("Arial", 16, "bold"))
        ...
        self.password_entry = ttk.Entry(main_frame, textvariable=self.password_var, width=60, show="*")
        ...
        self.login_btn = ttk.Button(login_frame, text="Se connecter avec identifiants", 
                                   command=self._login_with_credentials)
        ...
        ttk.Label(main_frame, text="Cookie (copié depuis Chrome):").grid(row=8, column=0, sticky=(tk.W, tk.N), pady=5)
        ...
        cookie_scroll = ttk.Scrollbar(cookie_frame, orient=tk.VERTICAL, command=self.cookie_text.yview)
        ...
        self.test_btn = ttk.Button(cookie_btn_frame, text="Tester connexion", 
                                  command=self._test_connection)
        self.test_btn.pack(pady=2)
        
        # Label statut connexion
        self.connection_label = ttk.Label(cookie_btn_frame, textvariable=self.connection_status_var,
                                         foreground="gray")
        self.connection_label.pack(pady=2)
        
        # === SECTION CHARGEMENT ===
        # Séparateur
        ttk.Separator(main_frame, orient='horizontal').grid(row=9, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10)
        
        # Bouton Charger
        self.load_btn = ttk.Button(main_frame, text="Charger mon emploi du temps", 
    ...
    def _update_login_success(self, cookies_string: str):
        ...
        messagebox.showinfo("Succès", "Connexion réussie ! Les cookies ont été remplis automatiquement.")
    ...
    def _update_login_failure(self, error_msg: str, status_code: str):
        ...
        messagebox.showerror("Échec de connexion", f"Impossible de se connecter:\n{error_msg}\n\nCode: {status_code}")
