...
class TestTimetableRegression(unittest.TestCase):
    ...
    def load_fixture(self, filename: str) -> str:
        ...
        self.assertTrue(
            fixture_path.exists(), 
            f"Fixture non trouvée: {fixture_path}"
        )
    ...
    def load_expected_result(self, filename: str) -> List[Dict[str, Any]]:
        ...
        self.assertTrue(
            ...
            f"Snapshot non trouvé: {snapshot_path}"
        )
    ...
    def test_sample_timetable_parsing(self):
        ...
        for i, course in enumerate(actual_result):
            self.assertIsInstance(
                course, dict, 
                f"Le cours à l'index {i} doit être un dictionnaire"
            )
            ...
            for field in critical_fields:
                self.assertIn(
                    field, course, 
                    f"Le champ '{field}' est manquant dans le cours {i}: {course}"
                )
    ...
    def test_parser_output_structure(self):
        ...
        for i, item in enumerate(result):
            self.assertIsInstance(
                item, dict,
                f"L'élément {i} doit être un dictionnaire, reçu: {type(item)}"
            )
